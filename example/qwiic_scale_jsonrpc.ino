...
void dispatch(const uint32_t id, const char *method, const JsonVariant& params) {
  ...
  else if (STRCMPI(method, "set_calibration")) {
    set_calibration(id, params);
  }
...
void calibrate(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<256> reply;

  float weight = params["calibration_weight"] | -1;
  int num_readings = params["average_size"] | -1;

  if ((weight < 1) || (weight > 500)) {
    jsonrpc_invalid_params(id, F("By-name parameter 'weight' is missing or outside range."));
    return;
  }
  else if ((num_readings < 1) || (num_readings > 255)) {
    jsonrpc_invalid_params(id, F("By-name parameter 'average_size' is missing or > 255."));
    return;
  }
...
void tare(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<128> reply;
  int num_readings = params["average_size"] | -1;

  if ((num_readings < 1) || (num_readings > 255)) {
    jsonrpc_invalid_params(id, F("By-name parameter 'average_size' is missing or > 255."));
    return;
  }
...
void change_mode(const uint32_t id, const JsonVariant& params) {
...
void set_calibration(const uint32_t id, const JsonVariant& params) {
  float cal_factor = params["cal_factor"] | 1.0f;
  long zero_offset = params["zero_offset"] | 0;
...
void get_calibration(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<128> reply;
  reply["id"] = id;
  JsonObject result = reply.createNestedObject("result");
  result["timestamp"] = millis();
...
void get_average_reading(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<128> reply;
  int num_readings = params["average_size"] | -1;
...
void get_average_weight(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<128> reply;
  int num_readings = params["average_size"] | -1;
  bool allow_negative = params["allow_negative"] | true;

  if ((num_readings < 1) || (num_readings > 255)) {
    jsonrpc_invalid_params(id, F("By-name parameter 'num_readings' is missing or > 255."));
    return;
  }
...
void get_status(const uint32_t id, const JsonVariant& params) {
  StaticJsonDocument<128> reply;
  reply["id"] = id;
  JsonObject result = reply.createNestedObject("result");
  result["timestamp"] = millis();
...
void jsonrpc_ack(const uint32_t id) {
  StaticJsonDocument<128> reply;
  reply["id"] = id;
  JsonObject result = reply.createNestedObject("result");
  result["timestamp"] = millis();
...
void jsonrpc_method_not_found(const uint32_t id) {
  StaticJsonDocument<128> reply;
  JsonObject err = reply.createNestedObject("error");
  err["timestamp"] = millis();
...
void jsonrpc_invalid_params(const uint32_t id, const __FlashStringHelper* data) {
  StaticJsonDocument<128> reply;
  JsonObject err = reply.createNestedObject("error");
  err["timestamp"] = millis();
  err["code"] = METHOD_NOT_FOUND;
...
void loop() {
  ...
  if (Serial.available()) {
    DeserializationError err = deserializeJson(request, Serial);
    if (err) {
